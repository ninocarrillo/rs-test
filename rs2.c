...
void InitRS2(int first_root, int num_roots, RS2_def_struct *rs) {
    ...
    for (i = first_root + 1; i < first_root + num_roots; i++) {
        factorpoly[0] = GF2Pow(i, rs->GF);
        GF2Conv(&rs->Genpoly[0], i + 1 - first_root, &factorpoly[0], 2, rs->GF);
    }
...
// calculate one syndrome for each root of rs->genpoly
	for (i = 0; i < rs->NumRoots; i++) {
		...
		for (j = 0; j < y; j++) {
			...
			syndromes[i] = GF2Mul(syndromes[i], x, rs->GF);
		}
		syndromes[i] = syndromes[i] ^ data_block[j];
	}
	...
	for (j = 0; j < block_size; j++) {
        ...
        for (i = 0; i < rs->ErrorCount; i++) {
            ...
            for (j = 1; j <= i; j++) {
                correction_poly[i] = correction_poly[i] ^ GF2Mul(syndromes[/*rs->FirstRoot + */i - j], error_locator[j], rs->GF);
            }
        }
