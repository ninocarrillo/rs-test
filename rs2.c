...
void RSEncode8(uint8_t *message, int16_t block_size, RS2_def_struct *rs) {
	int16_t i, j;
    uint8_t x;
	uint8_t quotient[MAX_GENPOLY_ROOTS + 1];
...
int16_t RSDecode8(uint8_t *data_block, int16_t block_size, RS2_def_struct *rs) {
...
// calculate one syndrome for each root of rs->genpoly
	for (i = 0; i < rs->NumRoots; i++) {
		...
		y = block_size - 1;
		for (j = 0; j < y; j++) {
			syndromes[i] = syndromes[i] ^ (uint16_t)data_block[j];
			syndromes[i] = gf2_mul(syndromes[i], x, rs->GF);
		}
		syndromes[i] = syndromes[i] ^ (uint16_t)data_block[j];
	}
    ...
    if (rs->ErrorCount <= ((rs->NumRoots / 2) - rs->MinimumErrorDistance)) {
        ...
        for (i = 0; i < rs->ErrorCount; i++) { // compute an error value for each error location
            ...
            y = gf2_pow(y, rs->GF);
            rs->ErrorMags[i] = gf2_mul(y, z, rs->GF);
            data_block[rs->ErrorLocs[i]] = data_block[rs->ErrorLocs[i]] ^ (uint8_t)(rs->ErrorMags[i] & 0xFF);
        }
        // check for success by calculating syndromes (should be zero if no errors)
        for (i = 0; i < rs->NumRoots; i++) {
            syndromes[i] = 0;
...
int16_t rs2_get_genpoly_coef(int16_t n, RS2_def_struct *rs) {
    return rs->Genpoly[n];
}